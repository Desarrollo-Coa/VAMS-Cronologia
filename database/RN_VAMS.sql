...
SELECT 
    ...
    p.PR_FECHA_FIN,
    p.PR_ACTIVO,
    NVL((SELECT COUNT(DISTINCT AV_IDACTIVO_PK) 
         FROM VMS_ACTIVO_VISUAL 
         WHERE PR_IDPROYECTO_FK = p.PR_IDPROYECTO_PK 
           AND AV_ACTIVO = 'SI'), 0) AS TOTAL_ACTIVOS,
    NVL((SELECT COUNT(DISTINCT CT_IDCATEGORIA_PK) 
         FROM VMS_CATEGORIA 
         WHERE PR_IDPROYECTO_FK = p.PR_IDPROYECTO_PK 
           AND CT_ACTIVO = 'SI'), 0) AS TOTAL_CATEGORIAS,
    (SELECT MAX(AV_FECHA_CARGA) 
     FROM VMS_ACTIVO_VISUAL 
     WHERE PR_IDPROYECTO_FK = p.PR_IDPROYECTO_PK 
       AND AV_ACTIVO = 'SI') AS ULTIMA_ACTUALIZACION
FROM VMS_PROYECTO p
